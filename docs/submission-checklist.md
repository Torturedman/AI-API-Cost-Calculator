...
这份文档用于提交前汇总 `plan.md` 要求的交付内容。当前项目已经完成核心单页工具、价格来源说明、英文 README、测试和构建验证；还需要补齐公网部署 URL、公开复盘博客链接，以及个人数字身份里无法从仓库确认的账号链接。
...
| 要求 | 当前状态 | 说明 |
| --- | --- | --- |
| 公网可访问 | 待完成 | 生产构建已可用，但还没有最终公网 URL |
...
| 移动端不崩 | 基本完成，待公网复验 | CSS 已做响应式布局和表格横向滚动；最终仍需在线上 URL 用手机或移动视口复验 |
...
1. 部署到 Vercel、Cloudflare Pages 或 Netlify，填入最终生产 URL。
2. 用匿名窗口打开生产 URL，确认不需要登录且返回可访问页面。
3. 用移动端或 360px 视口打开生产 URL，确认布局不乱、文本不溢出。
4. 把第 7 节复盘草稿公开发布，填入博客链接。
5. 补齐个人博客、X/Twitter 或其他技术社区账号；没有就如实填写“没有”。
...
- `npm run test -- --run` 通过：4 个测试文件、16 个测试全部通过。
